...
def visualize_prediction(model, val_loader, device, num_samples=1):
    model.eval()
    imgs, masks = next(iter(val_loader))  # take first batch
    ...
    for i in range(num_samples):
        ...
        img = imgs[i, 0].cpu().numpy()  # show FLAIR channel (index 0)

        mid_slice = img.shape[1] // 2  # axial slice in the middle
        ...
        plt.subplot(1, 3, 2)
        plt.imshow(gt[mid_slice], cmap="nipy_spectral")
        plt.title("Ground Truth")
        plt.axis("off")

        plt.subplot(1, 3, 3)
        plt.imshow(pred[mid_slice], cmap="nipy_spectral")
        plt.title("Prediction")
        plt.axis("off")

        plt.suptitle(f"Sample {i} — middle slice")
        plt.tight_layout()
        plt.show()
